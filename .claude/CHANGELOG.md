...
- 2026-06-12 feat(gateway): add `IN`/`ANY` filter operator — `filter[id][in]=a,b,c` (alias `filter[id][any]=...`) parses into `FilterOperator.IN` with a trimmed list of values and emits a single parameterized `WHERE id IN (?, ?, ?)`, replacing the N-request fan-out for batch lookups. List capped at `FilterCondition.MAX_IN_LIST_SIZE` (200); over-cap requests raise `InvalidQueryException` → HTTP 400. `EQ` deliberately keeps literal-value semantics (`filter[name][eq]=a,b,c` matches the literal string), with the multi-value path documented as IN-only (TASK-2026-06-12-0007)
