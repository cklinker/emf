...
- 2026-06-03 chore(runtime): populate JSON:API error envelope on every 4xx response — `code` now mandatory; added `@ExceptionHandler`s for `MethodArgumentNotValidException`, `ConstraintViolationException`, `HttpMessageNotReadableException`, `MissingServletRequestParameterException`, `MethodArgumentTypeMismatchException`, `NoHandlerFoundException`/`NoResourceFoundException`, `MethodNotAllowed`, `UnsupportedMediaType`, `ResponseStatusException`; fixed `JwtAuthenticationFilter` to emit `errors[]` array instead of singular `error` object (CHORE-2026-06-02-0008)
- 2026-06-03 doc(conventions): document JSON:API reserved field names (`type`, `id`, `attributes`, `relationships`, `links`, `meta` plus Kelta-internal `createdAt`/`updatedAt`/`createdBy`/`updatedBy`/`tenantId`), recommend entity-prefix workaround (`titleType`, `alertType`), and record the decision against auto-mangling (DOC-2026-06-02-0004)
- 2026-06-03 chore(api): MCP `add_field`/`update_field` accept native JSON `defaultValue` types (boolean/number/string), `create_record` and `update_record` responses echo caller-supplied `relationships`, and `create_collection`/`update_collection` expose `displayName` independently from `displayFieldName` (CHORE-2026-06-02-0013)
- 2026-06-03 chore(api): MCP `add_field`/`update_field` accept native JSON `defaultValue` types (boolean/number/string), `create_record` and `update_record` responses echo caller-supplied `relationships`, and `create_collection`/`update_collection` expose `displayName` independently from `displayFieldName` (CHORE-2026-06-02-0013)
