...
- 2026-06-03 chore(runtime): populate JSON:API error envelope on every 4xx response — `code` now mandatory; added `@ExceptionHandler`s for `MethodArgumentNotValidException`, `ConstraintViolationException`, `HttpMessageNotReadableException`, `MissingServletRequestParameterException`, `MethodArgumentTypeMismatchException`, `NoHandlerFoundException`/`NoResourceFoundException`, `MethodNotAllowed`, `UnsupportedMediaType`, `ResponseStatusException`; fixed `JwtAuthenticationFilter` to emit `errors[]` array instead of singular `error` object (CHORE-2026-06-02-0008)
- 2026-06-03 feat(runtime): filter orphan columns from JSON:API record responses — DELETE /api/fields/{id} only deprecates the underlying column (no DROP COLUMN), so SELECT * kept exposing deleted field names as null in data.attributes; DynamicCollectionRouter.toJsonApiResourceObject now drops keys that have no live FieldDefinition, no framework-metadata role, and no companion link to a still-live CURRENCY/GEOLOCATION field. Also fixes pre-existing McpApplicationTest expected-tools list to include delete_collection (TASK-2026-06-02-0005)
