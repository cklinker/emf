...
- 2026-06-12 feat(worker): validate `triggerConfig.cron` on flow save — `FlowScheduleSyncHook.beforeCreate`/`beforeUpdate` normalizes 5-field crons to Spring's 6-field form (prepends `0 ` for seconds) and rejects unparseable values with a 400 carrying `cron must be a 6-field Spring expression (with seconds); got '…'`, replacing the silent skip in `afterCreate` that left SCHEDULED flows with `0 */4 * * *` orphaned (no `scheduled_job` row). Normalization also flows through `ScheduledJobRepository.calculateNextRunAt` and the `/api/scheduled-jobs/{id}/resume` + `/validate-cron` endpoints via a new `CronExpressions` util. (TASK-2026-06-12-0001)
- 2026-06-12 feat(worker): validate `triggerConfig.cron` on flow save — `FlowScheduleSyncHook.beforeCreate`/`beforeUpdate` normalizes 5-field crons to Spring's 6-field form (prepends `0 ` for seconds) and rejects unparseable values with a 400 carrying `cron must be a 6-field Spring expression (with seconds); got '…'`, replacing the silent skip in `afterCreate` that left SCHEDULED flows with `0 */4 * * *` orphaned (no `scheduled_job` row). Normalization also flows through `ScheduledJobRepository.calculateNextRunAt` and the `/api/scheduled-jobs/{id}/resume` + `/validate-cron` endpoints via a new `CronExpressions` util. (TASK-2026-06-12-0001)
- 2026-06-12 feat(worker): expose scheduled-job status per flow — new `FlowScheduleController` adds `GET /api/flows/{id}/schedule` returning `{ cron, timezone, active, lastRunAt, lastStatus, nextRunAt, scheduleStatus }` and `GET /api/flows/{id}/runs` returning recent `job_execution_log` rows. The derived `scheduleStatus` is `UNSYNCED` (with `reason`) when a `flow_type=SCHEDULED` row has no matching `scheduled_job` or its `next_run_at IS NULL`, so the UI can warn "scheduled but not registered." Backed by new tenant-scoped repository methods `FlowRepository.findFlowTypeForTenant`, `ScheduledJobRepository.findScheduleForFlow`, and `findRecentRunsForFlow`. (TASK-2026-06-12-0003)
