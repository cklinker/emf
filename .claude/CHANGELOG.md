...
- 2026-06-12 feat(gateway): emit standard JSON:API `meta` alongside `metadata` on list responses — `DynamicCollectionRouter.toJsonApiListResponse` now writes the pagination map under both keys (same instance, identical content) so spec-compliant clients can read `meta.totalCount` while pre-existing `metadata` consumers keep working. `metadata` is documented as a deprecated alias (TASK-2026-06-12-0006)
