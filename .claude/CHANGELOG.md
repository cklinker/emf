...
- 2026-06-18 feat(runtime): add collection-name-aware overloads to `BeforeSaveHook` — `beforeCreate(collectionName, record, tenantId)` and `beforeUpdate(collectionName, id, record, previous, tenantId)` default to delegating to the existing no-name variants, so existing hooks continue to work unchanged. `BeforeSaveHookRegistry.evaluateBeforeCreate/Update` now dispatch through the aware variants, mirroring the existing `afterCreate/Update/Delete(collectionName, …)` pattern. Enables wildcard hooks (e.g. formula-field evaluation) to resolve behavior from collection-level metadata (TASK-2026-06-18-0002)
