...
- 2026-06-03 chore(runtime): populate JSON:API error envelope on every 4xx response — `code` now mandatory; added `@ExceptionHandler`s for `MethodArgumentNotValidException`, `ConstraintViolationException`, `HttpMessageNotReadableException`, `MissingServletRequestParameterException`, `MethodArgumentTypeMismatchException`, `NoHandlerFoundException`/`NoResourceFoundException`, `MethodNotAllowed`, `UnsupportedMediaType`, `ResponseStatusException`; fixed `JwtAuthenticationFilter` to emit `errors[]` array instead of singular `error` object (CHORE-2026-06-02-0008)
- 2026-06-03 chore(mcp): add admin tools `list_picklists`, `get_picklist`, `delete_picklist`, `add_picklist_value`, `update_picklist_value`, `deactivate_picklist_value` so setup scripts can inspect existing picklists and mutate values after creation; also adds `delete_collection` (added in #972) to the `McpApplicationTest` expected admin-tool surface (CHORE-2026-06-02-0010)
- 2026-06-03 chore(mcp): add admin tools `list_picklists`, `get_picklist`, `delete_picklist`, `add_picklist_value`, `update_picklist_value`, `deactivate_picklist_value` so setup scripts can inspect existing picklists and mutate values after creation; also adds `delete_collection` (added in #972) to the `McpApplicationTest` expected admin-tool surface (CHORE-2026-06-02-0010)
- 2026-06-03 feat(runtime): add `FieldType.TEXT` (plain long-form, Postgres TEXT) and `FieldType.VECTOR` (pgvector `vector(N)` with dimension from `fieldTypeConfig.dimension`, default 1536, range 1..16000); `RICH_TEXT` keeps its own identity in `SystemCollectionSeeder.mapFieldType` so the admin UI can distinguish rich-text editor vs plain textarea; `AddFieldTool` gains a `dimension` arg and rewrites the `richText` camelCase alias to canonical `rich_text` (TASK-2026-06-02-0012)
- 2026-06-03 feat(runtime): add `FieldType.TEXT` (plain long-form, Postgres TEXT) and `FieldType.VECTOR` (pgvector `vector(N)` with dimension from `fieldTypeConfig.dimension`, default 1536, range 1..16000); `RICH_TEXT` keeps its own identity in `SystemCollectionSeeder.mapFieldType` so the admin UI can distinguish rich-text editor vs plain textarea; `AddFieldTool` gains a `dimension` arg and rewrites the `richText` camelCase alias to canonical `rich_text` (TASK-2026-06-02-0012)
