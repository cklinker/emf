...
- 2026-06-12 feat(gateway): add `IN` / `ANY` filter operator — `filter[id][in]=a,b,c` (alias `filter[id][any]=...`) now parses to `FilterOperator.IN` with a trimmed list, replacing the N-request fan-out callers used to do for batch-by-id reads. List values use parameterized placeholders (no string interpolation); over-cap lists (>200 entries) raise `InvalidQueryException` → HTTP 400. `EQ` remains a literal-value match — multi-value filtering requires `IN` (TASK-2026-06-12-0007)
