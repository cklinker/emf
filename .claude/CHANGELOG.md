...
- 2026-06-03 feat(runtime): add composite (multi-column) unique constraint support — `POST/GET/DELETE /api/_composite-unique-constraints` and new `create_unique_constraint` MCP admin tool, backed by `PhysicalTableStorageAdapter.createCompositeUniqueConstraint` which emits `ALTER TABLE … ADD CONSTRAINT cuq_<table>_<crc> UNIQUE (…)`; duplicate-tuple inserts now surface as 409 with a `(field1, field2, …)` pointer instead of a generic 500 (TASK-2026-06-02-0007)
- 2026-06-03 feat(runtime): add composite (multi-column) unique constraint support — `POST/GET/DELETE /api/_composite-unique-constraints` and new `create_unique_constraint` MCP admin tool, backed by `PhysicalTableStorageAdapter.createCompositeUniqueConstraint` which emits `ALTER TABLE … ADD CONSTRAINT cuq_<table>_<crc> UNIQUE (…)`; duplicate-tuple inserts now surface as 409 with a `(field1, field2, …)` pointer instead of a generic 500 (TASK-2026-06-02-0007)
