...
- 2026-06-03 chore(mcp): point `create_picklist` tool at kebab-case `/api/global-picklists` and `/api/picklist-values`, attach values via flat `picklistSourceType=GLOBAL`+`picklistSourceId` attributes (was a 404-on-camelCase + invalid relationship payload) (CHORE-2026-06-02-0002)
- 2026-06-03 feat(runtime): add `FieldType.TEXT` and `FieldType.VECTOR`; map TEXT/RICH_TEXT to Postgres `TEXT` (distinct in `fields.type` so the admin UI renders rich-text vs textarea) and VECTOR to `vector(N)` via pgvector with `N` read from `fieldTypeConfig.dimension` (default 1536, range 1..16000); MCP `add_field` gains a `dimension` arg and rewrites the `richText` camelCase alias to `rich_text` (TASK-2026-06-02-0012)
- 2026-06-03 feat(runtime): add `FieldType.TEXT` and `FieldType.VECTOR`; map TEXT/RICH_TEXT to Postgres `TEXT` (distinct in `fields.type` so the admin UI renders rich-text vs textarea) and VECTOR to `vector(N)` via pgvector with `N` read from `fieldTypeConfig.dimension` (default 1536, range 1..16000); MCP `add_field` gains a `dimension` arg and rewrites the `richText` camelCase alias to `rich_text` (TASK-2026-06-02-0012)
- 2026-06-03 feat(runtime): formula parser now accepts `AND`/`OR`/`NOT` keywords as case-insensitive synonyms for `&&`/`||`/`!` so validation rules like `year < 1888 OR year > 2031` actually evaluate; `RecordValidationException` now returns HTTP 422 `VALIDATION_RULE_FAILED`; added `severity` field to `ValidationRuleDefinition` so `WARNING`-severity rules log without blocking; documented the formula grammar in `.claude/docs/conventions.md` (TASK-2026-06-02-0001)
