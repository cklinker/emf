...
- 2026-06-12 feat(gateway): add `IN`/`ANY` filter operator for collection queries — `filter[id][in]=a,b,c` (alias `[any]`) now parses the comma-separated value into a `List<String>` in `FilterCondition.fromParams`, the storage adapter emits a single parameterized `WHERE id IN (?, ?, ?)` (with per-element type coercion against the field type), and `metadata.totalCount` reflects the IN-filtered row count. List size is capped at `FilterCondition.MAX_IN_LIST_SIZE = 200`; over-cap requests raise `InvalidQueryException` → HTTP 400. `EQ` still treats a comma-separated value as one literal — multi-value matching requires `IN`/`ANY` (TASK-2026-06-12-0007)
- 2026-06-12 feat(gateway): add `IN`/`ANY` filter operator for collection queries — `filter[id][in]=a,b,c` (alias `[any]`) now parses the comma-separated value into a `List<String>` in `FilterCondition.fromParams`, the storage adapter emits a single parameterized `WHERE id IN (?, ?, ?)` (with per-element type coercion against the field type), and `metadata.totalCount` reflects the IN-filtered row count. List size is capped at `FilterCondition.MAX_IN_LIST_SIZE = 200`; over-cap requests raise `InvalidQueryException` → HTTP 400. `EQ` still treats a comma-separated value as one literal — multi-value matching requires `IN`/`ANY` (TASK-2026-06-12-0007)
