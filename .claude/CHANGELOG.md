...
- 2026-06-12 feat(gateway): surface page[size] clamp in JSON:API metadata — `DynamicCollectionRouter.toJsonApiListResponse` now adds `metadata.requestedPageSize` and `metadata.pageSizeClamped=true` whenever the caller's `page[size]` exceeded `MAX_HTTP_PAGE_SIZE`, so an over-cap request no longer looks like a silent-empty-data response (TASK-2026-06-12-0004)
- 2026-06-12 feat(flow): validate flow definition on save via `FlowDefinitionValidator` + worker `FlowDefinitionValidationHook` — POST/PUT `/api/flows` with a malformed `definition` (missing top-level `StartAt`, dangling `Next`/`Default`/`Choice.Next`/`Catch.Next`, undefined `StartAt` target, Map without iterator, nested branch/iterator with the same issues) now returns 400 enumerating every problem against field `definition` instead of accepting the record and crashing at execution time (TASK-2026-06-12-0002)
