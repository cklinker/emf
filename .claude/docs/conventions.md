...
- **Record detail = one `RecordShell`.** Both `ObjectDetailPage` (end-user) and `ResourceDetailPage` (admin) are thin `variant="enduser"|"admin"` wrappers over `RecordShell` (`kelta-ui/app/src/components/record/`), which owns the page skeleton (loading/status + breadcrumb→header→(sectionNav+body+rail)→tabBar→belowTabs→dialogs slots; `sectionNav` is the sticky left `RecordSectionNav` — layout-section jump list + Activity entry — and the `ActivityTimeline` renders inside `body` below the sections, anchor `record-activity`, not in `belowTabs`); the field body goes through `RecordDetailBody` (`LayoutFieldSections` when the layout has sections, else a variant fallback slot). **Do not add per-page detail bodies or a third section renderer** — extend `RecordShell`/`RecordDetailBody` (add a slot) so both stacks stay in sync. The kelta-web `LayoutRenderer` is a separate legacy public export (its related-list is a display-only stub); it is not on this path — don't wire new record-detail work through it.
