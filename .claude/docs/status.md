...
| Governed AI agents | **Definition model + CRUD**: `AgentDefinition` (system prompt, allowed MCP tool subset, model/maxTokens/monthly-token-budget overrides, enabled) stored in `ai_agent` (kelta-ai Flyway V4, tenant-scoped); `AgentService` + `AgentDefinitionRepository` (JdbcTemplate) + `AgentController` CRUD at `/api/ai/agents` | orchestration runtime (tool-use loop), per-tool Cerbos authz + audit, PII masking, agent-builder UI (next Rec 3 slices 6b–6e) |
