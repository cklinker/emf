...
| Page builder / screen builder | `PageBuilderPage` editor + CRUD; `ui-pages` has `slug`/`published`/`config` (V116); **render contract**: `GET /api/pages/{slug}/render` (`PageRenderController`/`PageRenderService`) serves a published+active page as a **versioned** `PageRenderContract` (`{version, slug, title, path, tree}`); gateway static route `/api/pages/**` registered (Rec 1 slice 1a); **builder persists the component tree into the `config` JSON column** (slice 1b — previously `components`/`layout` were sent as top-level attributes and silently dropped by the worker, so saved pages lost their tree); **end-user renderer**: `CustomPage` (route `/:tenant/app/p/:slug`) fetches the render contract and renders the tree via the schema-driven `PageTreeRenderer` (heading/text/button/image/card/container/table/form → React, plugin components via `componentRegistry`) — slice 1c; **`UIPageConfigEventPublisher` broadcasts `kelta.config.page.changed.<pageId>`** on page create/update/delete for multi-pod cache invalidation (slice 1d); **table data binding**: a `table` node with `props.dataView = {collection, fields, limit}` renders live rows — `DataTableNode` fetches `/api/{collection}` via the authorized JSON:API path (gateway+worker enforce Cerbos/FLS; denied fields stripped server-side) and shows the declared columns (slice 1e-render); **builder PropertyPanel data-source editor** for table+form (collection/fields/limit → `props.dataView`, slice 1e-ui); **form create binding**: a `form` node renders an input per declared field and submits a new record via `apiClient.postResource('/api/{collection}', …)` — the authorized JSON:API create path (Cerbos/FLS enforced) — slice 1f | per-page Cerbos authz (published+active+tenant is the visibility gate); typed/validated form fields (text-only inputs today); positive Playwright e2e once deployed |
| External-entity storage (connectors) | **Dispatch seam (Rec 4 slice 4a)**: `StorageAdapter` gained `default String storageType()`; `DispatchingStorageAdapter` (`@Primary`, runtime-core) routes every storage op to the adapter backing the target collection via `storageConfig().adapterConfig()["adapterType"]` (absent/unknown → `PhysicalTableStorageAdapter`; fail-safe fallback never silently mis-routes); external backends implement the `ExternalStorageAdapter` marker + override `storageType()`. Zero behavior change today (no external adapter registered). Full runtime-core suite (1325) green incl. context boot | no concrete `ExternalRest`/`ExternalJdbc` adapter yet (4b/4c); no persistence of `adapterConfig` on the `collection` table (transient — needs a JSONB column + load/store); no connector packs or admin UI; virtual-collection import not wired |
