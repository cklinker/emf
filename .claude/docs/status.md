...
| Page builder / screen builder | `PageBuilderPage` editor + CRUD; `ui-pages` has `slug`/`published`/`config` (V116); **render contract**: `GET /api/pages/{slug}/render` (`PageRenderController`/`PageRenderService`) serves a published+active page as a **versioned** `PageRenderContract` (`{version, slug, title, path, tree}`); gateway static route `/api/pages/**` registered (Rec 1 slice 1a); **builder persists the component tree into the `config` JSON column** (slice 1b — previously `components`/`layout` were sent as top-level attributes and silently dropped by the worker, so saved pages lost their tree); **end-user renderer**: `CustomPage` (route `/:tenant/app/p/:slug`) fetches the render contract and renders the tree via the schema-driven `PageTreeRenderer` (heading/text/button/image/card/container/table/form → React, plugin components via `componentRegistry`) — slice 1c; **`UIPageConfigEventPublisher` broadcasts `kelta.config.page.changed.<pageId>`** on page create/update/delete for multi-pod cache invalidation (slice 1d) | data binding for table/form nodes via a `DataView` saved query + a builder property editor for them (no data-source UI yet); per-page Cerbos authz (currently published+active+tenant is the visibility gate); positive Playwright e2e once deployed |
