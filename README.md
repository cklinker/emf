...
| `RATE_LIMIT_IP_PATHS` | Optional — per-IP budgets for unauthenticated paths as `<path-prefix>=<per-minute>`, matched by longest prefix, each prefix its own bucket. Default `/actuator/health=100,/api/billing/webhooks=300` |
