...
| `RATE_LIMIT_USER_SHARE` | Optional — fraction of a tenant's rate-limit window any single member may consume (default `0.5`). Bounds one member (or one stolen PAT) from locking out their whole tenant. `1.0` turns the per-user window off; an out-of-range value falls back to the default rather than disabling it |
