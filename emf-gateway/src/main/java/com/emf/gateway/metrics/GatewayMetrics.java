package com.emf.gateway.metrics;
...
/**
 * Centralized metrics service for the EMF API Gateway.
 *
 ...
 * <ol>
 *   <li>{@code emf.gateway.requests} — Timer (tenant, method, status, route)</li>
 *   <li>{@code emf.gateway.requests.active} — Gauge (tenant)</li>
 *   <li>{@code emf.gateway.auth.failures} — Counter (tenant, reason)</li>
 *   <li>{@code emf.gateway.authz.denied} — Counter (tenant, route, method)</li>
 *   <li>{@code emf.gateway.ratelimit.exceeded} — Counter (tenant)</li>
 *   <li>{@code emf.gateway.ratelimit.remaining.ratio} — Gauge (tenant)</li>
 *   <li>{@code emf.gateway.permissions.resolve} — Timer (tenant, source)</li>
 *   <li>{@code emf.gateway.tenant.resolution} — Counter (method, result)</li>
 *   <li>{@code emf.gateway.errors} — Counter (tenant, status, error_code)</li>
 * </ol>
 */
@Component
public class GatewayMetrics {
    ...
    public void recordRequest(String tenant, String method, String status, String route, Duration duration) {
        Timer.builder("emf.gateway.requests")
    ...
    // -----------------------------------------------------------------------
    // 2. emf.gateway.requests.active (Gauge)
    // -----------------------------------------------------------------------
    ...
    private AtomicInteger getOrCreateActiveGauge(String tenant) {
        return activeRequests.computeIfAbsent(tenant, t -> {
            ...
            registry.gauge("emf.gateway.requests.active", io.micrometer.core.instrument.Tags.of(TAG_TENANT, t), gauge);
            return gauge;
        });
    }

    // -----------------------------------------------------------------------
    // 3. emf.gateway.auth.failures (Counter)
    // -----------------------------------------------------------------------
    ...
    public void recordAuthFailure(String tenant, String reason) {
        Counter.builder("emf.gateway.auth.failures")
                .tag(TAG_TENANT, safe(tenant))
    ...
    // -----------------------------------------------------------------------
    // 4. emf.gateway.authz.denied (Counter)
    // -----------------------------------------------------------------------
    ...
    public void recordAuthzDenied(String tenant, String route, String method) {
        Counter.builder("emf.gateway.authz.denied")
                .tag(TAG_TENANT, safe(tenant))
                .tag(TAG_ROUTE, safe(route))
                .tag(TAG_METHOD, safe(method))
                .register(registry)
    ...
    // -----------------------------------------------------------------------
    // 5. emf.gateway.ratelimit.exceeded (Counter)
    // -----------------------------------------------------------------------
    ...
    public void recordRateLimitExceeded(String tenant) {
        Counter.builder("emf.gateway.ratelimit.exceeded")
                .tag(TAG_TENANT, safe(tenant))
                .register(registry)
                .increment();
    }
    ...
    // -----------------------------------------------------------------------
    // 6. emf.gateway.ratelimit.remaining.ratio (Gauge)
    // -----------------------------------------------------------------------
    ...
    private AtomicInteger getOrCreateRateLimitGauge(String tenant) {
        return rateLimitRemainingPct.computeIfAbsent(tenant, t -> {
            AtomicInteger gauge = new AtomicInteger(0);
            registry.gauge("emf.gateway.ratelimit.remaining.ratio",
    ...
    // -----------------------------------------------------------------------
    // 7. emf.gateway.permissions.resolve (Timer)
    // -----------------------------------------------------------------------
    ...
    public void recordPermissionResolve(String tenant, String source, Duration duration) {
        Timer.builder("emf.gateway.permissions.resolve")
                .tag(TAG_TENANT, safe(tenant))
    ...
    // -----------------------------------------------------------------------
    // 8. emf.gateway.tenant.resolution (Counter)
    // -----------------------------------------------------------------------
    ...
    public void recordTenantResolution(String method, String result) {
        Counter.builder("emf.gateway.tenant.resolution")
                .tag(TAG_METHOD, safe(method))
    ...
    public void recordError(String tenant, String status, String errorCode) {
        Counter.builder("emf.gateway.errors")
                .tag(TAG_TENANT, safe(tenant))
                .tag(TAG_STATUS, safe(status))
