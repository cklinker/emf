...
describe('AuthorizationPanel', () => {
  ...
  describe('Route-Level Authorization (Requirements 5.9, 5.10)', () => {
    ...
    it('displays policy select for each route operation', () => {
      ...
      ROUTE_OPERATIONS.forEach((operation) => {
        expect(screen.getByTestId(`authorization-panel-route-${operation}-select`)).toBeInTheDocument();
      });
    });
    ...
    it('shows current route policy selection from authz config', () => {
      ...
      const createSelect = screen.getByTestId('authorization-panel-route-create-select') as HTMLSelectElement;
      const readSelect = screen.getByTestId('authorization-panel-route-read-select') as HTMLSelectElement;
    ...
    it('shows empty selection for operations without configured policy', () => {
      ...
      const updateSelect = screen.getByTestId('authorization-panel-route-update-select') as HTMLSelectElement;
      const deleteSelect = screen.getByTestId('authorization-panel-route-delete-select') as HTMLSelectElement;
  ...
  describe('Field-Level Authorization (Requirements 5.11, 5.12)', () => {
    ...
    it('displays read and write operations for expanded field', async () => {
      ...
      FIELD_OPERATIONS.forEach((operation) => {
        expect(screen.getByTestId(`authorization-panel-field-field-1-${operation}`)).toBeInTheDocument();
      });
    });
    ...
    it('displays policy select for each field operation', async () => {
      ...
      FIELD_OPERATIONS.forEach((operation) => {
        expect(screen.getByTestId(`authorization-panel-field-field-1-${operation}-select`)).toBeInTheDocument();
      });
    });
    ...
    it('shows current field policy selection from authz config', async () => {
      ...
      const readSelect = screen.getByTestId('authorization-panel-field-field-2-read-select') as HTMLSelectElement;
      const writeSelect = screen.getByTestId('authorization-panel-field-field-2-write-select') as HTMLSelectElement;
    ...
    it('calls onFieldAuthzChange when field policy is selected', async () => {
      ...
      renderWithI18n(
        <AuthorizationPanel 
          {...defaultProps} 
          onFieldAuthzChange={onFieldAuthzChange}
        />
      );
    ...
    it('collapses field when clicked again', async () => {
      ...
      // Collapse
      await user.click(fieldToggle);
      expect(screen.queryByTestId('authorization-panel-field-field-1-operations')).not.toBeInTheDocument();
    });
  ...
  describe('No Policies Warning', () => {
    ...
    it('does not show warning when policies are available', () => {
      ...
      expect(screen.queryByTestId('authorization-panel-no-policies-warning')).not.toBeInTheDocument();
    });
  });
  ...
  describe('Edge Cases', () => {
    it('handles undefined authz gracefully', () => {
      ...
      ROUTE_OPERATIONS.forEach((operation) => {
        const select = screen.getByTestId(`authorization-panel-route-${operation}-select`) as HTMLSelectElement;
