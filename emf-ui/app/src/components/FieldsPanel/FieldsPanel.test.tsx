...
const sampleFields: FieldDefinition[] = [
  createField({ id: 'field-1', name: 'name', displayName: 'Name', type: 'string', required: true, order: 0 }),
  createField({ id: 'field-2', name: 'email', displayName: 'Email', type: 'string', unique: true, order: 1 }),
  createField({ id: 'field-3', name: 'age', displayName: 'Age', type: 'number', indexed: true, order: 2 }),
  createField({ id: 'field-4', name: 'is_active', displayName: 'Is Active', type: 'boolean', order: 3 }),
  createField({ id: 'field-5', name: 'created_at', displayName: 'Created At', type: 'datetime', order: 4 }),
];
...
describe('FieldsPanel Component', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should render field display name when different from name', () => {
      ...
      expect(screen.getByTestId('fields-panel-field-display-name-field-1')).toHaveTextContent('(Name)');
    });
    ...
    it('should not render display name when same as name', () => {
      ...
      expect(screen.queryByTestId('fields-panel-field-display-name-field-1')).not.toBeInTheDocument();
    });
    ...
    it('should render reference target for reference fields', () => {
      const fields = [
        createField({ id: 'field-1', name: 'user_id', type: 'reference', referenceTarget: 'users', order: 0 }),
      ];
      renderWithI18n(<FieldsPanel {...defaultProps} fields={fields} />);

      expect(screen.getByTestId('fields-panel-field-reference-field-1')).toHaveTextContent('→ users');
    });
  ...
  describe('Field Types Display', () => {
    it('should display all field types correctly', () => {
      const allTypeFields: FieldDefinition[] = [
        ...
        createField({ id: 'f7', name: 'reference_field', type: 'reference', referenceTarget: 'users', order: 6 }),
      ];
  ...
  it('should work with state management for CRUD operations', () => {
    const TestComponent = () => {
      const [fields, setFields] = React.useState<FieldDefinition[]>([
        { id: '1', name: 'field1', type: 'string', required: false, unique: false, indexed: false, order: 0 },
        { id: '2', name: 'field2', type: 'number', required: false, unique: false, indexed: false, order: 1 },
      ]);
