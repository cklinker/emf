...
export function AuthProvider({
   ...
   */
  const handleCallback = useCallback(async (availableProviders: OIDCProviderSummary[]): Promise<void> => {
    ...
    if (!provider) {
      console.error('[Auth] Provider not found:', providerId, 'Available:', availableProviders.map(p => p.id));
      throw new Error(`Provider not found: ${providerId}`);
    }
    ...
    if (newUser) {
      setUser(newUser);
      console.log('[Auth] User set successfully:', { id: newUser.id, email: newUser.email, name: newUser.name });
    } else {
      ...
      try {
        ...
        if (isCallback && code) {
          ...
          if (processedCode === code) {
            console.log('[Auth] Callback already processed for this code, skipping duplicate (React StrictMode)');
            setIsLoading(false);
            return;
          }
        ...
        if (isCallback) {
          ...
          try {
            ...
            setError(callbackError instanceof Error ? callbackError : new Error('Authentication failed'));
