...
function MockPageComponent({ config }: PageComponentProps) {
  return (
    <div data-testid="page-component">
      Page Component: {JSON.stringify(config)}
    </div>
  );
}
...
describe('PluginContext', () => {
  ...
  describe('Plugin Lifecycle Hooks (Requirement 12.8)', () => {
    ...
    it('should pass plugin context to onLoad hook', async () => {
      ...
      expect(typeof (receivedContext as Record<string, unknown>).registerFieldRenderer).toBe('function');
      expect(typeof (receivedContext as Record<string, unknown>).registerPageComponent).toBe('function');
