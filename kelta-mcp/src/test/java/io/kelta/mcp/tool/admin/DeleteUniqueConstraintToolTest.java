...
class DeleteUniqueConstraintToolTest {
    ...
    private static final String INDEX_NAME = "uniq_availabilities_title_provider";
    ...
    void deletesConstraint() {
        wm.stubFor(delete(urlEqualTo(
                "/api/admin/collections/availabilities/unique-constraints/" + INDEX_NAME))
                .willReturn(aResponse().withStatus(204)));

        CallToolResult result = tool.toSpecification().callHandler().apply(
                null, new CallToolRequest("delete_unique_constraint",
                        Map.of("collectionName", "availabilities",
                                "indexName", INDEX_NAME), null));

        assertThat(result.isError()).isNotEqualTo(Boolean.TRUE);
        assertThat(((TextContent) result.content().get(0)).text())
                .contains("Deleted unique constraint", INDEX_NAME, "204");
    ...
    void surfacesGatewayErrorOnNotFound() {
        wm.stubFor(delete(urlEqualTo(
                "/api/admin/collections/availabilities/unique-constraints/" + INDEX_NAME))
                .willReturn(aResponse().withStatus(404).withBody("{\"error\":\"Constraint not found\"}")));
