...
import type { CreateDashboardRequest } from '@kelta/sdk'
...
function MetricCardWidget({ component, apiClient }: WidgetRendererProps): React.ReactElement {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () =>
      apiClient.getList<Record<string, unknown>>(`/api/${collection}?page[size]=10000`),
    enabled: !!collection,
    staleTime: 30000,
  })
...
function DataTableWidget({ component, apiClient }: WidgetRendererProps): React.ReactElement {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () =>
      apiClient.getList<Record<string, unknown>>(`/api/${collection}?page[size]=${rowLimit}`),
    enabled: !!collection,
    staleTime: 30000,
  })
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () =>
      apiClient.getList<Record<string, unknown>>(
        `/api/${collection}?sort=-createdAt&page[size]=${rowLimit}`
      ),
    enabled: !!collection,
    staleTime: 30000,
  })
...
function DashboardViewer({
  ...
  const {
    ...
    queryFn: () => keltaClient.admin.dashboards.get(dashboardId) as Promise<Dashboard>,
    enabled: !!dashboardId,
  })
...
function DashboardEditorInner({
  ...
  const saveMutation = useMutation({
    mutationFn: (components: DashboardComponent[]) =>
      keltaClient.admin.dashboards.update(dashboard.id, {
        name: dashboard.name,
        description: dashboard.description,
...
export function DashboardsPage({
  ...
  const {
    ...
    queryKey: ['dashboards'],
    queryFn: () => keltaClient.admin.dashboards.list() as Promise<Dashboard[]>,
  })
  ...
  const createMutation = useMutation({
    mutationFn: (data: DashboardFormData) =>
      keltaClient.admin.dashboards.create(
        '',
        '',
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: DashboardFormData }) =>
      keltaClient.admin.dashboards.update(
        id,
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => keltaClient.admin.dashboards.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['dashboards'] })
