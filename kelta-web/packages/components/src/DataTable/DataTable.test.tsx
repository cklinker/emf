...
import type { KeltaClient, ListResponse } from '@kelta/sdk';
...
const createWrapper = (client: KeltaClient) => {
  ...
  return ({ children }: { children: React.ReactNode }) => (
    <QueryClientProvider client={queryClient}>
      <KeltaProvider client={client}>{children}</KeltaProvider>
    </QueryClientProvider>
  );
};
...
describe('DataTable', () => {
  ...
  describe('Data Fetching (Requirement 11.1)', () => {
    it('should fetch data using KeltaClient on mount', async () => {
  ...
  describe('Keyboard Navigation', () => {
    it('should navigate rows with arrow keys', async () => {
      ...
      expect(rows[1]).toHaveClass('kelta-datatable__row--focused');
    });
