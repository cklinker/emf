...
import type { User } from '@kelta/sdk';
...
describe('Navigation', () => {
  ...
  describe('Active State (Requirement 15.4)', () => {
    it('should highlight active menu item based on current route', () => {
      ...
      expect(usersItem).toHaveClass('kelta-navigation__item--active');

      const homeItem = screen.getByText('Home').closest('li');
      expect(homeItem).not.toHaveClass('kelta-navigation__item--active');
    });
    ...
    it('should highlight parent when child route is active', async () => {
      ...
      expect(settingsItem).toHaveClass('kelta-navigation__item--active');
    });
    ...
    it('should handle exact match for root path', () => {
      ...
      const homeItem = screen.getByText('Home').closest('li');
      expect(homeItem).toHaveClass('kelta-navigation__item--active');

      // Users should not be active
      const usersItem = screen.getByText('Users').closest('li');
      expect(usersItem).not.toHaveClass('kelta-navigation__item--active');
    });
  });
  ...
  describe('Keyboard Navigation', () => {
    it('should navigate with arrow keys', async () => {
      ...
      const usersButton = screen.getByText('Users').closest('button')!;
      expect(usersButton).toHaveClass('kelta-navigation__link--focused');
    });
  ...
  describe('Orientation', () => {
    it('should apply horizontal orientation class by default', () => {
      ...
      expect(screen.getByTestId('kelta-navigation')).toHaveClass('kelta-navigation--horizontal');
    });
    ...
    it('should apply vertical orientation class when specified', () => {
      ...
      expect(screen.getByTestId('kelta-navigation')).toHaveClass('kelta-navigation--vertical');
    });
  ...
  describe('Accessibility', () => {
    ...
    it('should apply custom className', () => {
      ...
      expect(screen.getByTestId('kelta-navigation')).toHaveClass('custom-nav');
    });
    ...
    it('should apply collapsible class when enabled', () => {
      ...
      expect(screen.getByTestId('kelta-navigation')).toHaveClass('kelta-navigation--collapsible');
    });
  });
});
