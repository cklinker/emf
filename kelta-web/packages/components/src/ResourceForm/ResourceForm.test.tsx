  ...
  describe('Field Generation (Requirement 12.2)', () => {
    ...
    it('should mark required fields with asterisk', async () => {
      ...
      await waitFor(() => {
        ...
        expect(nameLabel?.querySelector('.kelta-resource-form__required')).toBeInTheDocument();

        const ageLabel = screen.getByText('Age').closest('label');
        expect(ageLabel?.querySelector('.kelta-resource-form__required')).not.toBeInTheDocument();
      });
    });
  });
