...
import {
  TODAY, NOW, ISBLANK, BLANKVALUE, IF, AND_FN, OR_FN, NOT_FN,
  LEN, CONTAINS, UPPER, LOWER, TRIM, TEXT, VALUE,
  ROUND, ABS, MAX_FN, MIN_FN, REGEX, DATEDIFF, ALL_BUILTINS, buildBuiltinMap,
} from './builtins';
...
describe('BLANKVALUE', () => {
  ...
  it('throws on missing args', () => expect(() => BLANKVALUE.execute([null], ctx)).toThrow(FormulaException));
});
...
describe('IF', () => {
  ...
  it('throws on missing args', () => expect(() => IF.execute([true], ctx)).toThrow(FormulaException));
});
...
describe('AND/OR/NOT', () => {
  ...
  it('NOT throws on no args', () => expect(() => NOT_FN.execute([], ctx)).toThrow(FormulaException));
});
...
describe('CONTAINS', () => {
  ...
  it('throws on missing arg', () => expect(() => CONTAINS.execute(['a'], ctx)).toThrow(FormulaException));
});
...
describe('TEXT/VALUE', () => {
  ...
  it('VALUE bad string throws', () => expect(() => VALUE.execute(['abc'], ctx)).toThrow(FormulaException));
});
...
describe('ROUND/ABS/MAX/MIN', () => {
  ...
  it('ROUND throws on missing places', () => expect(() => ROUND.execute([3.14], ctx)).toThrow(FormulaException));
  ...
  it('MAX throws on missing arg', () => expect(() => MAX_FN.execute([3], ctx)).toThrow(FormulaException));
});

describe('REGEX', () => {
  it('matches whole string', () => expect(REGEX.execute(['abc', '[a-z]+'], ctx)).toBe(true));
  it('does not match partial (anchored)', () => expect(REGEX.execute(['abc123', '[a-z]+'], ctx)).toBe(false));
  it('matches with anchor implied', () => expect(REGEX.execute(['abc123', '[a-z]+[0-9]+'], ctx)).toBe(true));
  it('throws on missing arg', () => expect(() => REGEX.execute(['abc'], ctx)).toThrow(FormulaException));
});
...
describe('DATEDIFF', () => {
  ...
  it('throws on missing arg', () => expect(() => DATEDIFF.execute(['2026-01-01'], ctx)).toThrow(FormulaException));
  it('throws on invalid date', () => expect(() => DATEDIFF.execute(['nope', '2026-01-01'], ctx)).toThrow(FormulaException));
});
