...
public class SpanIdCaptureInterceptor implements HandlerInterceptor {
    ...
    public static final String SERVER_SPAN_ID_ATTR = "kelta.server.spanId";
    public static final String SERVER_TRACE_ID_ATTR = "kelta.server.traceId";
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (span.getSpanContext().isValid()) {
            ...
            setIfPresent(span, "kelta.user.id", request.getHeader("X-User-Id"));
            setIfPresent(span, "kelta.user.email", request.getHeader("X-Forwarded-User"));
            setIfPresent(span, "kelta.tenant.id", request.getHeader("X-Tenant-ID"));
            setIfPresent(span, "kelta.tenant.slug", request.getHeader("X-Tenant-Slug"));
            setIfPresent(span, "kelta.correlation.id", request.getHeader("X-Correlation-ID"));
        }
        return true;
    }
