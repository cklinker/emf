...
class WorkerMetricsConfigTest {
    ...
    void shouldRegisterActiveCollectionsGauge() {
        Gauge gauge = meterRegistry.find("kelta.worker.collections.active").gauge();
        assertNotNull(gauge, "kelta.worker.collections.active gauge should be registered");
    ...
    void shouldRegisterCollectionCountGaugeForHpa() {
        Gauge gauge = meterRegistry.find("kelta.worker.collection.count").gauge();
        assertNotNull(gauge, "kelta.worker.collection.count gauge should be registered for HPA");
        assertEquals(5.0, gauge.value(), "Should reflect lifecycle manager collection count");
    }
    ...
    void shouldRegisterInitializingCollectionsGauge() {
        Gauge gauge = meterRegistry.find("kelta.worker.collections.initializing").gauge();
        assertNotNull(gauge, "kelta.worker.collections.initializing gauge should be registered");
    ...
    void shouldRegisterUptimeGauge() {
        Gauge gauge = meterRegistry.find("kelta.worker.uptime.seconds").gauge();
        assertNotNull(gauge, "kelta.worker.uptime.seconds gauge should be registered");
        assertTrue(gauge.value() >= 0, "Uptime should be non-negative");
    }

    @Test
    void shouldRegisterHeapUsedGauge() {
        Gauge gauge = meterRegistry.find("kelta.worker.heap.used.bytes").gauge();
        assertNotNull(gauge, "kelta.worker.heap.used.bytes gauge should be registered");
        assertTrue(gauge.value() > 0, "Heap used should be positive");
    }

    @Test
    void shouldRegisterHeapMaxGauge() {
        Gauge gauge = meterRegistry.find("kelta.worker.heap.max.bytes").gauge();
        assertNotNull(gauge, "kelta.worker.heap.max.bytes gauge should be registered");
