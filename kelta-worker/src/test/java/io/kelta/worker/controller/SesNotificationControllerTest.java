package io.kelta.worker.controller;

import io.kelta.worker.repository.EmailRepository;
import io.kelta.worker.repository.EmailSuppressionRepository;
import org.junit.jupiter.api.BeforeAll;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;
import org.springframework.http.HttpStatus;
import org.springframework.http.MediaType;
import org.springframework.http.ResponseEntity;
import org.springframework.test.web.client.MockRestServiceServer;
import org.springframework.web.client.RestClient;
import tools.jackson.databind.ObjectMapper;
import tools.jackson.databind.node.ObjectNode;

import java.nio.charset.StandardCharsets;
import java.security.KeyFactory;
import java.security.PrivateKey;
import java.security.Signature;
import java.security.spec.PKCS8EncodedKeySpec;
import java.util.Base64;
import java.util.Optional;

import static org.assertj.core.api.Assertions.assertThat;
import static org.mockito.ArgumentMatchers.any;
import static org.mockito.ArgumentMatchers.eq;
import static org.mockito.Mockito.mock;
import static org.mockito.Mockito.never;
import static org.mockito.Mockito.verify;
import static org.mockito.Mockito.when;
import static org.springframework.test.web.client.response.MockRestResponseCreators.withStatus;

/**
 * Same throwaway test cert/key as {@code SnsSignatureVerifierTest} — see there for provenance.
 * Regression coverage for two bugs found on review of the original PR: an UnsubscribeConfirmation
 * must not auto-resubscribe (it would undo a deliberate operator unsubscribe), and a "not-spam"
 * complaint feedback type must not suppress (it's a retraction, not a new complaint).
 */
@DisplayName("SesNotificationController")
class SesNotificationControllerTest {

    private static final String CERT_URL = "https://sns.us-east-1.amazonaws.com/test-cert.pem";
    private static final String TOPIC_ARN = "arn:aws:sns:us-east-1:123:spotopened-ses-notifications";
    private static final ObjectMapper MAPPER = new ObjectMapper();

    private static final String TEST_CERT_PEM = """
            -----BEGIN CERTIFICATE-----
            
            -----END CERTIFICATE-----
            """;

    private static final String TEST_KEY_PKCS8_BASE64 = """
            
            """;

    private static byte[] certPem;
    private static PrivateKey privateKey;

    private EmailSuppressionRepository suppressionRepository;
    private EmailRepository emailRepository;
    private MockRestServiceServer server;
    private SesNotificationController controller;

    @BeforeAll
    static void loadKeyMaterial() throws Exception {
        certPem = TEST_CERT_PEM.getBytes(StandardCharsets.UTF_8);
        byte[] keyBytes = Base64.getMimeDecoder().decode(TEST_KEY_PKCS8_BASE64);
        privateKey = KeyFactory.getInstance("RSA").generatePrivate(new PKCS8EncodedKeySpec(keyBytes));
    }

    @BeforeEach
    void setUp() {
        suppressionRepository = mock(EmailSuppressionRepository.class);
        emailRepository = mock(EmailRepository.class);
        RestClient.Builder builder = RestClient.builder();
        server = MockRestServiceServer.bindTo(builder).build();
        controller = new SesNotificationController(builder.build(), suppressionRepository,
                emailRepository, MAPPER);
    }

    private void expectCertFetch() {
        server.expect(org.springframework.test.web.client.match.MockRestRequestMatchers.requestTo(CERT_URL))
                .andRespond(withStatus(HttpStatus.OK).body(certPem).contentType(MediaType.TEXT_PLAIN));
    }

    private String sign(String stringToSign) throws Exception {
        Signature sig = Signature.getInstance("SHA256withRSA");
        sig.initSign(privateKey);
        sig.update(stringToSign.getBytes(StandardCharsets.UTF_8));
        return Base64.getEncoder().encodeToString(sig.sign());
    }

    private String notificationToSign(String message, String messageId, String timestamp, String topicArn) {
        return "Message\n" + message + "\nMessageId\n" + messageId + "\nTimestamp\n" + timestamp
                + "\nTopicArn\n" + topicArn + "\nType\nNotification\n";
    }

    private String unsubscribeToSign(String message, String messageId, String subscribeUrl,
                                      String timestamp, String token, String topicArn) {
        return "Message\n" + message + "\nMessageId\n" + messageId + "\nSubscribeURL\n" + subscribeUrl
                + "\nTimestamp\n" + timestamp + "\nToken\n" + token + "\nTopicArn\n" + topicArn
                + "\nType\nUnsubscribeConfirmation\n";
    }

    private ObjectNode envelope(String type, String message, String messageId, String timestamp,
                                 String topicArn, String signature) {
        ObjectNode node = MAPPER.createObjectNode();
        node.put("Type", type);
        node.put("Message", message);
        node.put("MessageId", messageId);
        node.put("Timestamp", timestamp);
        node.put("TopicArn", topicArn);
        node.put("SignatureVersion", "2");
        node.put("Signature", signature);
        node.put("SigningCertURL", CERT_URL);
        return node;
    }

    @Test
    @DisplayName("does not hit SubscribeURL for an UnsubscribeConfirmation (would undo a deliberate unsubscribe)")
    void doesNotAutoResubscribe() throws Exception {
        expectCertFetch();
        String subscribeUrl = "https://sns.us-east-1.amazonaws.com/?Action=ConfirmSubscription&Token=t";
        String toSign = unsubscribeToSign("hello", "msg-1", subscribeUrl, "2026-08-28T00:00:00Z", "tok-1", TOPIC_ARN);
        String signature = sign(toSign);

        ObjectNode body = MAPPER.createObjectNode();
        body.put("Type", "UnsubscribeConfirmation");
        body.put("Message", "hello");
        body.put("MessageId", "msg-1");
        body.put("SubscribeURL", subscribeUrl);
        body.put("Timestamp", "2026-08-28T00:00:00Z");
        body.put("Token", "tok-1");
        body.put("TopicArn", TOPIC_ARN);
        body.put("SignatureVersion", "2");
        body.put("Signature", signature);
        body.put("SigningCertURL", CERT_URL);

        ResponseEntity<Void> response = controller.receive(MAPPER.writeValueAsString(body));

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        // Only the cert-fetch call is expected; hitting subscribeUrl would fail verify() below
        // since no matcher was registered for it.
        server.verify();
    }

    @Test
    @DisplayName("does not suppress on a 'not-spam' complaint feedback type (a retraction, not a new complaint)")
    void doesNotSuppressOnNotSpamRetraction() throws Exception {
        expectCertFetch();
        when(emailRepository.findTenantIdByFromAddress("noreply@spotopened.com"))
                .thenReturn(Optional.of("tenant-1"));

        String innerMessage = MAPPER.writeValueAsString(java.util.Map.of(
                "notificationType", "Complaint",
                "mail", java.util.Map.of("source", "noreply@spotopened.com"),
                "complaint", java.util.Map.of(
                        "complaintFeedbackType", "not-spam",
                        "complainedRecipients", java.util.List.of(
                                java.util.Map.of("emailAddress", "someone@example.com")))));

        String toSign = notificationToSign(innerMessage, "msg-2", "2026-08-28T00:00:00Z", TOPIC_ARN);
        String signature = sign(toSign);
        ObjectNode body = envelope("Notification", innerMessage, "msg-2", "2026-08-28T00:00:00Z", TOPIC_ARN, signature);

        ResponseEntity<Void> response = controller.receive(MAPPER.writeValueAsString(body));

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        verify(suppressionRepository, never()).add(any(), any(), any(), any(), any());
    }

    @Test
    @DisplayName("still suppresses on a genuine complaint (no feedback type, or one other than 'not-spam')")
    void suppressesOnGenuineComplaint() throws Exception {
        expectCertFetch();
        when(emailRepository.findTenantIdByFromAddress("noreply@spotopened.com"))
                .thenReturn(Optional.of("tenant-1"));

        String innerMessage = MAPPER.writeValueAsString(java.util.Map.of(
                "notificationType", "Complaint",
                "mail", java.util.Map.of("source", "noreply@spotopened.com"),
                "complaint", java.util.Map.of(
                        "complainedRecipients", java.util.List.of(
                                java.util.Map.of("emailAddress", "someone@example.com")))));

        String toSign = notificationToSign(innerMessage, "msg-3", "2026-08-28T00:00:00Z", TOPIC_ARN);
        String signature = sign(toSign);
        ObjectNode body = envelope("Notification", innerMessage, "msg-3", "2026-08-28T00:00:00Z", TOPIC_ARN, signature);

        controller.receive(MAPPER.writeValueAsString(body));

        verify(suppressionRepository).add(eq("tenant-1"), eq("someone@example.com"), eq("COMPLAINT"), eq(null), eq(null));
    }
}
